...
sc = Scattering1D(**configs, frontend='numpy')
...
plot(Scx[order0_idxs], show=1, title="Time scattering | Zeroth order")
# show modulus and disable interpolation in few-sample regime (along time axis)
ikw = dict(abs=1, interpolation='none')
imshow(Scx[order1_idxs], **ikw, title="Time scattering | First order")
imshow(Scx[order2_idxs], **ikw, title="Time scattering | Second order, unrolled")
