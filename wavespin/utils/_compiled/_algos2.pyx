...
cpdef int smallest_interval_over_threshold2(double[:] x, double threshold, int c=-1):
    ...
    cdef double sm = x[left]

    # handle `left <= c < right` constraint
    if c != -1:
        # sum up to `c`
        for right in prange(1, c + 1, nogil=True):
            sm += x[right]
...
@cython.boundscheck(False)
@cython.nonecheck(False)
cpdef tuple smallest_interval_over_threshold_indices2(
    double[:] x, double threshold, int c, int interval):
    # initialize variables ###################################################
    cdef Py_ssize_t N = x.shape[0]
    ...
    for idx in prange(left + 1, right, nogil=True):
        sm += x[idx]
