...
from .scat_utils import _compute_minimum_support_to_pad
...
class _FrequencyScatteringBase1D(ScatteringBase):
    ...
    def __init__(self, paths_include_build, N_frs, J_fr=None, Q_fr=None, F=None,
        ...
        # TODO scattering constructors show kymatio
        # TODO doc N_frs_min_global default
        # TODO is fit_smart_paths doing as perc of each pair? (JTFS)
    ...
    def build(self):
        ...
        # check `J_fr`, `Q_fr`, `F`  # TODO F?
        _ensure_positive_integer(self, ('J_fr', 'Q_fr'))
        ...
        if self.F == 'global':
            ...
            raise ValueError("The temporal support F of the low-pass filter "
                             ...
                             "(got {} > {})".format(
                                 self.F, 2**self.N_fr_scales_max))
        self.log2_F = max(math.floor(math.log2(self.F)), 0)  # TODO `max(, 0)` ?
        ...
        # handle `max_pad_factor_fr` #########################################
        # restrict `J_pad_frs_max` (and `J_pad_frs_max_init`) if specified by user
        if isinstance(self.max_pad_factor_fr, int):
            # see docs
            ideal = max(4 + int(np.ceil(np.log2(self.width_exclude_ratio))),
                        1)

            def convert(scale_diff):
                if scale_diff == 0:
                    # definition
                    mpf_psi = mpf_phi = self.max_pad_factor_fr
                else:
                    mpf_psi = mpf_phi = self.max_pad_factor_fr + scale_diff
                    if self.sampling_psi_fr != 'resample':
                        mpf_psi = max(min(mpf_psi, ideal - 1), 1)
                    if self.sampling_phi_fr != 'resample':
                        mpf_phi = max(min(mpf_phi, ideal - 1), 1)
                return (mpf_psi, mpf_phi)

            self.max_pad_factor_fr = {
                scale_diff: convert(scale_diff)
            ...
            if max(self.max_pad_factor_fr.values()) > self.max_pad_factor_fr[0]:
                ...
                # the lowest that `max_pad_factor_fr[0]` can be to ensure
                # `J_pad_frs[0] > J_pad_frs[1:]`
                first_max_pf_min = J_pad_frs_max - self.N_fr_scales_max
                self.max_pad_factor_fr[0] = first_max_pf_min

            # convert to `list[tuple[int]]`
            for k, v in self.max_pad_factor_fr.items():
                self.max_pad_factor_fr[k] = (v, v)
        ...
        if (self.J_fr == self.N_fr_scales_max and
                ...
                max(self.max_pad_factor_fr[0]) == 0):
        ...
        (self.J_pad_frs_max_init, self._pad_fr_psi, self._pad_fr_phi
         ) = self._compute_J_pad_fr(2**self.N_fr_scales_max)
    ...
    def create_phi_filters(self):
        """See `filter_bank_jtfs.phi_fr_factory`."""
        self.phi_f_fr = phi_fr_factory(
            self.J_pad_frs_max_init, self.J_pad_frs_phi, self.F, self.log2_F,
    ...
    def create_psi_filters(self):
        """See `filter_bank_jtfs.psi_fr_factory`."""
    ...
    def _compute_psi_fr_params(self):
        ...
        if self.sampling_psi_fr in ('resample', 'exclude'):
            ...
            for scale_diff in params_new['j']:
                if (self.scale_diff_max_recalibrate is not None and
                        scale_diff > self.scale_diff_max_recalibrate):
                    break
                params[scale_diff] = {field: [] for field in params_init}
    ...
    def compute_padding_fr(self):
        ...
        -------------------
          ...
          - `ind_end_fr_max`: maximum unpad index across all `n2` for a given
            ...
            (Note, `ind_end_fr` in above example having greater entries in
            non-`ind_end_fr[0]` should never happen, it's shown illustratively.)
        """
        self.J_pad_frs = {}
        self.J_pad_frs_phi = {}
        ...
        for N_fr_scale in self.N_fr_scales_unique[::-1]:
            ...
            if (self.scale_diff_max_to_build is not None and
                ...
                if self.J_pad_frs_min_limit_due_to_psi is None:
                    self.J_pad_frs_min_limit_due_to_psi = self.J_pad_frs[
                        scale_diff]
                self.J_pad_frs_phi[scale_diff] = self.J_pad_frs_phi[
                    self.scale_diff_max_to_build]
                continue
            ...
            if self.unrestricted_pad_fr:
                ...
                pad_boundeffs_phi = min_to_pad_bound_effs_phi
            else:
                mpf_psi, mpf_phi = self.max_pad_factor_fr[scale_diff]
                pad_boundeffs_psi = min(min_to_pad_bound_effs_psi,
                                        N_fr_scale + mpf_psi)
                pad_boundeffs_phi = min(min_to_pad_bound_effs_phi,
                                        N_fr_scale + mpf_phi)
            ...
            s_spinned = max(self.total_conv_stride_over_U1s[scale_diff])
            s_phi     = self.total_conv_stride_over_U1s_phi[scale_diff]
            min_to_pad_stride_psi = s_spinned
            ...
            if self.average_fr:
                J_pad_fr = max(J_pad_fr_psi, J_pad_fr_phi)
                J_pad_fr_phi = J_pad_fr
            else:
                # `J_pad_fr` controls spinned
                J_pad_fr = J_pad_fr_psi
                # `J_pad_fr_phi` controls `phi_t * psi_f`; we discard the
                # unimportant case of `J_pad_fr_phi < J_pad_fr_psi`; also
                # ensure we don't exceed `J_pad_frs_max`
                J_pad_fr_phi = max(J_pad_fr_psi, J_pad_fr_phi)
        ...
        for k, v in self.J_pad_frs_phi.items():
            self.J_pad_frs_phi[k] = min(max(self.J_pad_frs.values()), v)

        # ensure integer type (non-numpy to not confuse backends)
        for pad_frs in (self.J_pad_frs, self.J_pad_frs_phi):
            for k, v in pad_frs.items():
                pad_frs[k] = int(v)

        # ensure psi never pads more than phi, per current implementation
        for ppsi, pphi in zip(self.J_pad_frs.values(),
                              self.J_pad_frs_phi.values()):
            assert ppsi <= pphi, (self.J_pad_frs, self.J_pad_frs_phi)
    ...
    def _get_min_to_pad(self, N_fr_scale, init=False):
        """See "Limitation" in `scat_utils.compute_minimum_support_to_pad`.

        Width accounting in `phi` padding
        ---------------------------------
        `*4` then `/2` is for clarity: we seek to account for x4 the width,
        at which point (x2 the width from peak to either side) the filter is
        ~1/4 its peak. But to do so for sake of spatial expansiveness, we only
        need to use half the spatial amount, as the measure is "amount of
        effective space added"^1, rather than "amount of space needed to prevent
        filter from drawing circularly from input". Yet, one directly ties to the
        other; "amount of space needed" is computed from input size, and
        "effective space added" increases the effective input size^2.

          - 1: measured as distance from peak of filter to "edge" of input,
            as that's the most contributing part of filter to output (since
            our filters are strictly decaying).
            In detail: imagine a filter centered at edge of input, then moving
            away; while centered, the most it draws from any one point is
            maximal, but once moving, it's determined by the last point
            touching the edge, which is measured as distance from peak to that
            point, rather than distance from one such point to the other
            (reflection over peak) in the filter
          - 2: though the measure here is much more conservative: "x2 width" is
            much smaller than "support". Hence, `None` padding isn't perfect
            with respect to higher-order convolutions. This is to avoid excessive
            pad requirements.
        """
        # make reusable params
        N_init = 2**N_fr_scale
        scale_diff = self.N_fr_scales_max - N_fr_scale
        halve_padding = bool(self.pad_mode_fr == 'zero' and self.halve_zero_pad)
        ca = dict(criterion_amplitude=self.criterion_amplitude)
        common_kw = dict(N_init=N_init, normalize=self.normalize_fr,
                         criterion_amplitude=self.criterion_amplitude,
                         P_max=self.P_max, eps=self.eps)

        # handle `init`
        if init:
            J_pad = None  # see docs in `compute_minimum_support_to_pad`
            sigma_low, xi1_fr, sigma1_fr, *_ = calibrate_scattering_filters(
                self.J_fr, self.Q_fr, self.F, r_psi=self.r_psi_fr,
                sigma0=self.sigma0, J_pad=J_pad)
        ...
        # psi ################################################################
        if init:
            min_to_pad_psi = _compute_minimum_support_to_pad(
                xi=xi1_fr[-1], sigma=sigma1_fr[-1], halve_padding=halve_padding,
                **common_kw)
            ...
            min_to_pad_psi = _compute_minimum_support_to_pad(
                xi=xis[-1], sigma=sigmas[-1], halve_padding=halve_padding,
                **common_kw)

        # phi ################################################################
        reuse_pad_phi = bool(not init and
                             self.sampling_phi_fr == 'resample')
        if reuse_pad_phi:
            ...
            if self.average_fr_global_phi:
                ...
                # `halve_padding=False` (below) to avoid `//= 2` since this is
                # potentially a non-final step (per `_adj_pad_phi`)
                if init:
                    min_to_pad_phi = _compute_minimum_support_to_pad(
                        sigma=sigma_low, halve_padding=False, **common_kw)
                    ...
                    min_to_pad_phi = _compute_minimum_support_to_pad(
                        sigma=sigma_low_F, halve_padding=False, **common_kw)

                if halve_padding:
                    # non-final step; avoid `//= 2`
                    min_to_pad_phi /= 2

                # Account for spatial expansiveness of convolution; add width of
                # largest psi. -----------------------------------------------
                # fetch params
                if init:
                    xis, sigmas = xi1_fr, sigma1_fr
                else:
                    xis, sigmas = [self.psi_fr_params[scale_diff][field]
                                   for field in ('xi', 'sigma')]

                # make psi-maker
                psi_fn = lambda N: morlet_1d(
                    N, xis[-1], sigmas[-1], normalize=self.normalize_fr,
                    **self.get_params('P_max', 'eps', 'precision'))
                # get lowest `N` for ideal sampling
                ...
                min_to_pad_phi += (4 * psi_max_width) / 2
                min_to_pad_phi = int(min_to_pad_phi)
                # ------------------------------------------------------------
        return min_to_pad_psi, min_to_pad_phi
    ...
    def _compute_J_pad_fr(self, N_fr):
        ...
        min_to_pad_psi, min_to_pad_phi = self._get_min_to_pad(
            N_fr_scale, init=True)
        min_to_pad = max(min_to_pad_psi, min_to_pad_phi)
        ...
        if not self.unrestricted_pad_fr:
            ...
            # this method's only used output is `J_pad_frs_max_init`, and that
            # should set the largest filter length
        ...
        return J_pad, min_to_pad_psi, min_to_pad_phi
    ...
    def _assert_nonincreasing_J_pad_frs(self):
        for pad_frs in (self.J_pad_frs, self.J_pad_frs_phi):
            ...
            assert all(p <= self.J_pad_frs_max_init for p in pad_frs.values()
                       ), (pad_frs, self.J_pad_frs_max_init)
...
def phi_fr_factory(J_pad_frs_max_init, J_pad_frs_phi, F, log2_F,
    ...
    pads_iterated = []
    for J_pad_fr in list(J_pad_frs_phi.values())[::-1]:
        # avoid recomputation
...
def _recalibrate_psi_fr(max_original_width, xi1_frs, sigma1_frs, j1_frs,
    ...
    n_cqt = sum(is_cqt1_frs)
    n_noncqt = len(is_cqt1_frs) - n_cqt
    ...
    def make_logspace(mn, mx, n):
        return np.logspace(np.log10(mn), np.log10(mx), n, endpoint=True)[::-1]
    ...
    for N_fr_scale in N_fr_scales_unique[::-1]:
        ...
        # CQT first
        new_sigma_cqt = make_logspace(new_sigma_min, sigma_max, n_cqt)

        r = new_sigma_cqt[1] / new_sigma_cqt[0]
        # halve distance from existing xi_max to .5 (max possible)
        new_xi_max = .5 - (.5 - xi_max) / factor
        new_xi_min = new_xi_max * r**(n_cqt - 1)
        new_xi_cqt = make_logspace(new_xi_min, new_xi_max, n_cqt)

        # non-CQT now
        new_sigma_noncqt = new_sigma_min * np.ones(n_noncqt)
        new_xi_noncqt = np.linspace(new_xi_min/(n_noncqt + 1), new_xi_min,
                                    n_noncqt, endpoint=False)[::-1]

        # combine
        new_sigma = np.hstack([new_sigma_cqt, new_sigma_noncqt])
        new_xi = np.hstack([new_xi_cqt, new_xi_noncqt])
